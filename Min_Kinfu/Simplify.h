...
namespace Simplify
{
	...
	void simplify_mesh(int target_count, double agressiveness=7, bool verbose=false)
	{
		...
		for (int iteration = 0; iteration < 100; iteration ++)
		{
			...
			//double threshold = 0.0000000001*pow(double(iteration), agressiveness);
	...
	*/
	void importMesh(std::vector<Eigen::Vector3f> &vertices_mesh, std::vector<int> &indices)
	{
		vertices.clear();
		triangles.clear();
		...
		{
			Vertex v;
			v.p.x = vertices_mesh.at(i)[0];
			v.p.y = vertices_mesh.at(i)[1];
			v.p.z = vertices_mesh.at(i)[2];
	...
	created at 01/21/2019
	*/
	void exportMesh(std::vector<Eigen::Vector3f> &vertices_mesh, std::vector<int> &indices)
	{
		for(int i=0; i < vertices.size(); ++i)
		{
			...
			v[1] = vertices[i].p.y;
			v[2] = vertices[i].p.z;
			vertices_mesh.push_back(v);
		}
		for(int i=0; i < triangles.size(); ++i)
		{
			indices.push_back(triangles[i].v[0]);
			indices.push_back(triangles[i].v[1]);
			indices.push_back(triangles[i].v[2]);
		}
	}
	//Option : Load OBJ
